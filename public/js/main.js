...
function aboutView() {
  ...
  const meta = state.lang === 'zh' ? [
    ...
    { l: '操作级别', v: 'B 类' },
    { l: '网格定位', v: 'OM86' },
    { l: '邮箱', v: 'grainrain2012@outlook.com', mail: true },
    { l: '备用邮箱', v: 'bg4lzn@qq.com', mail: true }
  ] : [
    { l: 'Callsign', v: 'BG4LZN' },
    { l: 'Location', v: "Tai'an, Shandong, China" },
    { l: 'License', v: 'Class B' },
    { l: 'Grid Square', v: 'OM86' },
    { l: 'Email', v: 'grainrain2012@outlook.com', mail: true },
    { l: 'Alt Email', v: 'bg4lzn@qq.com', mail: true }
  ];
...
function qslView() {
  ...
  <div style="display:flex;gap:10px;margin-bottom:14px;">
    <input id="qslSearch" type="search" placeholder="${escapeHtml(pick('检索呼号 / 波段 / 备注…', 'Search callsign / band / note…'))}" style="flex:1;padding:10px 14px;border:1px solid var(--line);border-radius:8px;background:var(--bg);color:var(--ink);font-size:14px;">
  </div>
  <div class="card" style="padding:18px;overflow-x:auto;">
    <table class="logs" style="width:100%;">
      <thead><tr><th>${escapeHtml(pick('收卡时间', 'Received'))}</th><th>${escapeHtml(pick('呼号', 'Callsign'))}</th><th>${escapeHtml(pick('波段', 'Band'))}</th><th>${escapeHtml(pick('模式', 'Mode'))}</th><th>${escapeHtml(pick('状态', 'Status'))}</th><th>${escapeHtml(pick('备注', 'Note'))}</th></tr></thead>
...
async function renderQsl() {
  ...
  try {
    ...
    const render = () => {
      statEl.innerHTML = [
        `<span class="band-chip" style="font-size:14px;">🃏 ${escapeHtml(rows.length)} / ${escapeHtml(all.length)} ${escapeHtml(pick('条', 'records'))}</span>`,
      ...
      listEl.innerHTML = rows.length ? rows.map(q => `
        ...
        </tr>`).join('') : `<tr><td colspan="6" style="text-align:center;color:var(--ink-faint);">${escapeHtml(pick(rows === all ? '暂无 QSL 记录' : '无匹配结果', rows === all ? 'No QSL records yet' : 'No matches'))}</td></tr>`;
    };
    const box = qs('#qslSearch');
    if (box) {
      box.addEventListener('input', () => {
        const kw = box.value.trim().toUpperCase();
        rows = kw ? all.filter(q => String(q.call || '').toUpperCase().includes(kw) || String(q.band || '').toUpperCase().includes(kw) || String(q.mode || '').toUpperCase().includes(kw) || String(q.note || '').toUpperCase().includes(kw)) : all;
        render();
      });
    }
    render();
...
function applyStaticI18n() {
  ...
  if (brand) brand.textContent = pick('BG4LZN 的电波小窝', 'BG4LZN\'s Radio Nook');
